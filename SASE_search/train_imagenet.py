...
from model_attention_imgnet import resnet50, resnet101

parser = argparse.ArgumentParser("imagenet")
parser.add_argument('--arch', type=str, default='res50', help='Please indicate model type')
parser.add_argument('--workers', type=int, default=16, help='number of workers to load dataset')
parser.add_argument('--data', type=str, default='/location/of/imagenet-1k/', help='location of the data corpus')
...
parser.add_argument('--lr_scheduler', type=str, default='cosine', help='lr scheduler, step or cosine')
...
def main():
    ...
    # model = Network(CLASSES, genotype)
    if args.arch == 'res50':
        model = resnet50()
    ...
    if args.lr_scheduler == 'step':
        ...
        logging.info('Chosen scheduler: step LR' )
    else:
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, float(args.epochs))
        logging.info('Chosen scheduler: cosine annealing' )
    best_acc_top1 = 0
    best_acc_top5 = 0
    ...
    for epoch in range(args.epochs):
        ...
        '''
        # Apply warmup only for large batch sizes
